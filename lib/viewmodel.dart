import 'model.dart';

class FortuneViewModel {
  // 0～14 までノード 15 個（質問 7 個 + 結果 8 個）
  final List<Node?> node = List<Node?>.filled(15, null);

  int _currentIndex = 0;

  FortuneViewModel() {
    // ----- 質問ノード (0〜6) -----
    node[0] = QuestionNode("あなたは行動的ですか？", 1, 2);               // 深さ1
    node[1] = QuestionNode("初対面の人と話すのは得意ですか？", 3, 4);     // 深さ2 (YES側)
    node[2] = QuestionNode("一人で過ごす時間が好きですか？", 5, 6);       // 深さ2 (NO側)

    node[3] = QuestionNode("計画を立ててから動くほうですか？", 7, 8);    // 深さ3
    node[4] = QuestionNode("思いついたらすぐ行動に移しますか？", 9, 10); // 深さ3
    node[5] = QuestionNode("コツコツと同じ作業を続けられますか？", 11, 12); // 深さ3
    node[6] = QuestionNode("気分によって行動が変わりやすいほうですか？", 13, 14); // 深さ3

    // ----- 結果ノード (7〜14) -----
    node[7]  = ResultNode("🔥", "エネルギータイプ", "情熱的でリーダーシップを発揮しやすいタイプです。");
    node[8]  = ResultNode("🎯", "アクションタイプ", "思い切りの良さでチャンスをつかむタイプです。");
    node[9]  = ResultNode("💡", "アイデアタイプ", "ひらめきと好奇心が強いクリエイティブなタイプです。");
    node[10] = ResultNode("🤝", "バランスタイプ", "周りとの調和を大事にする安定感のあるタイプです。");
    node[11] = ResultNode("🌱", "マイペースタイプ", "落ち着いていて着実に前に進む努力家タイプです。");
    node[12] = ResultNode("🧱", "安定タイプ", "慎重で堅実、周囲から信頼されるタイプです。");
    node[13] = ResultNode("🌙", "クリエイティブタイプ", "感性が豊かで表現することが得意なタイプです。");
    node[14] = ResultNode("☁️", "マイワールドタイプ", "自分の世界観を大切にする自由なタイプです。");
  }

  Node get currentNode => node[_currentIndex] as Node;
  bool get isFinished => currentNode is ResultNode;

  void restart() => _currentIndex = 0;

  void answer(bool yes) {
    final q = currentNode as QuestionNode;
    _currentIndex = yes ? q.yes : q.no;
  }
}
